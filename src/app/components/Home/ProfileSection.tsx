...
import useAPI, { UseFetchReturnType } from '@/app/hooks/useAPI'

const ProfileSection = () => {
  const [image, setImage] = useState('')
  const [online, setOnline] = useState(false)
  const { data, isLoading }: UseFetchReturnType = useAPI(config.profile_section.discord_api, 10000)
  useEffect(() => {
          ...
          <div className='w-[88%] mx-auto'>
            ...
            <div>
              ...
              <div className='w-full flex flex-row items-center'>
                <Image
                  src={data?.data.listening_to_spotify ? data?.data?.spotify?.album_art_url as string : 'https://us-east-1.tixte.net/uploads/data_of_omega.tixte.co/no_music.png'}
                  alt='album_photo'
                  width={55}
                  height={55}
                ...
                <div>
                  <h5 className='text-[0.65rem] font-extrabold text-profile_section_description_major'>
                    {data?.data.listening_to_spotify ? data?.data?.spotify?.song as string : 'No song is playing.'}
                  </h5>
                  <h6 className='text-[0.65rem] font-medium text-profile_section_description_minor'>
                    {data?.data.listening_to_spotify ? `By ${data?.data?.spotify?.artist as string}` : 'By no one.'}
                  </h6>
                  <h6 className='text-[0.65rem] font-medium text-profile_section_description_minor'>
                    {data?.data.listening_to_spotify ? `In ${data?.data?.spotify?.album as string}` : 'In no album.'}
                  </h6>
                </div>
              </div>
            </div>
